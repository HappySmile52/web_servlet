...
public class encry_model {
	public String dataencode(String word) {	// base64 암호화
		...
		String security = ec.encodeToString(word.getBytes());	// byte 단위
		return security;
	}
	...
	// MD5 : %02x 기본 => %x로 써도 %02x => %03x 부터는 자릿수가 변경되는 사항 (x : 소문자, X : 대문자)
	// sha-1 : 16진수 40자로 생성되는 암호화 기술 (%01x, %02x 완전 다름)
	...
	public String md5_encode(String word) {
		...
		try {
			...
			byte md5byte[] = md.digest();	// 원시 배열에 해당 암호화된 byte 저장

...
public class join_ok extends HttpServlet {
	...
	Connection con = null;	// Database 연결 (SQL Query문 실행)
	
	/*
	// doGet, doPost = service(모두 사용 가능)
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("POST,GET 모두 받는 역할");
	}
	*/
	...
	*/
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		catch(SQLIntegrityConstraintViolationException ie) {	// DB에 unique가 중복값이 발생하였을 경우
			this.pw.write("<script>"
					+ "alert('연락처 및 이메일이 중복 되어서 가입이 취소되었습니다.');"
					+ "history.go(-1);"
					+ "</script>");
		}
		catch(Exception e) {

...
public class event_infook extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			int result = this.st.executeUpdate(sql);	// executeUpdate는 insert, update, delete에서만 사용
			if(result == 1) {
		...
		finally {	// 정상적인 Database를 모두 핸들링 후 역순으로 close를 순차적으로 적용해야 함
			try {
				this.pw.close();
				...
				System.out.println("Database가 올바르게 종료되지 않았습니다.");
			}
		}
	}
}
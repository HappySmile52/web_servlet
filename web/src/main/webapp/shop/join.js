	...
	if(frm.sid.value == ""){
		...
		if(frm.spart[0].checked == true){	// 일반 회원
			if(document.getElementById("idck").value == ""){
				alert("아이디 중복체크를 해야만 회원가입이 진행됩니다.");
			}
			else{
				frm.submit();
			}
...
function ajaxpost(data){
	...
	function wck(){
		...
		if(window.XMLHttpRequest){	// (XHR통신)
			return new XMLHttpRequest();	// 신규 XHR 통신을 생성함
		}
	}
		...
		*/
		if(ok.readyState == XMLHttpRequest.DONE && ok.status == 200){
			...
			if(this.response == "ok"){
				...
				frm.sid.readOnly = true;	// 아이디를 더 이상 수정하지 못하도록 읽기 전용으로 변경
			}
	...
	// ok.open("통신규약","Back-end 경로",ture:비동기통신)
	// ok.open("통신규약","Back-end 경로",false:동기통신)
